# Architecture

**Claude Code Deep Research Agent** - 系统架构文档 (v3.1)

> 📘 **相关文档**: [CLAUDE.md](CLAUDE.md) | [RESEARCH_METHODOLOGY.md](RESEARCH_METHODOLOGY.md)

---

## 设计原则

### 核心理念

```
图思维 > 线性流程
智能迭代 > 固定阶段
服务器处理 > 客户端处理
硬约束 > 软约束
```

### 架构目标

1. **智能性** - GoT 动态路径优化，而非固定流程
2. **高效性** - 服务器端批处理，减少 LLM 调用
3. **可靠性** - 硬预算强制，自动错误修复
4. **可观测性** - 完整的图状态追踪

---

## 系统架构

### v3.1 分层架构图

```
┌─────────────────────────────────────────────────────────────┐
│                        用户接口层                             │
│              Commands: /deep-research [--fast]              │
│         (模式检测: Quick | Deep)                             │
└──────────────────────────┬──────────────────────────────────┘
                           │ invokes
┌──────────────────────────▼──────────────────────────────────┐
│                   GoT 控制器 (Agent)                         │
│  ┌────────────────────────────────────────────────────────┐ │
│  │     research-coordinator-v3 (Graph Navigator)          │ │
│  │                                                        │ │
│  │  Generate → Execute → Score → Prune → Aggregate → Loop │ │
│  │                                                        │ │
│  │  终止条件: confidence ≥ 0.9 OR budget exhausted        │ │
│  └────────────────────────────────────────────────────────┘ │
│                                                              │
│  部署 research-worker-v3 agents (并行探索路径)                │
└──────────────────────────┬──────────────────────────────────┘
                           │ uses
┌──────────────────────────▼──────────────────────────────────┐
│                   基础设施层 (Infrastructure)                 │
│  ┌──────────────┐  ┌───────────────┐                       │
│  │ MCP Tools    │  │ GoT State Mgmt│                       │
│  │ (数据处理)    │  │ (Graph Ctrl)  │                       │
│  └──────────────┘  └───────────────┘                       │
│  ┌──────────────────────────────────────────────────┐      │
│  │        SQLite (sessions, paths, metrics)          │      │
│  └──────────────────────────────────────────────────┘      │
└─────────────────────────────────────────────────────────────┘
```

### 架构演变

| 版本 | 工作流 | 处理 | 控制 | 层级 |
|------|--------|------|------|------|
| **v2.1** | 线性 Phase 1-7 | Agent 侧 | 软约束 | 4 层 |
| **v3.0** | 线性 Phase 1-7 | Agent 侧 | 软约束 | 2 层 |
| **v3.1** | GoT 循环 | 服务器侧 | 硬约束 | 2 层 |

---

## 目录结构 (v3.1)

```
.
├── .claude/
│   ├── commands/                  # 用户命令
│   │   └── deep-research-v3.md    # v3.1 命令 (支持 --fast)
│   │
│   ├── agents/                    # 代理定义 (简化)
│   │   ├── research-coordinator-v3/
│   │   │   └── AGENT.md           # GoT 控制器
│   │   ├── research-worker-v3/
│   │   │   └── AGENT.md           # 路径执行者
│   │   └── data-processor-v3/
│   │       └── AGENT.md           # 数据处理器
│   │
│   └── mcp-server/                # MCP 服务器
│       ├── src/
│       │   ├── tools/
│       │   │   ├── auto-process-data.ts  # v3.1 新增
│       │   │   ├── got-tools.ts          # GoT 操作
│       │   │   ├── batch-tools.ts        # 批处理
│       │   │   └── ...
│       │   ├── logic/
│       │   │   ├── graph-controller.ts   # v3.1 增强
│       │   │   └── types.ts
│       │   └── index.ts
│       └── data/
│           └── research_state.db
│
├── RESEARCH/                      # 研究输出
│   └── [topic]/
│       ├── README.md
│       ├── executive_summary.md
│       ├── full_report.md
│       ├── data/
│       │   ├── raw/               # 原始数据
│       │   ├── processed/         # 服务器处理
│       │   └── got_graph.json     # GoT 状态 (v3.1)
│       └── ...
│
├── CLAUDE.md                      # 快速参考
├── ARCHITECTURE.md                # 本文档
├── RESEARCH_METHODOLOGY.md        # 研究方法论
└── README.md
```

---

## Graph of Thoughts (GoT) 架构

### 核心概念

**传统线性流程** vs **图思维**:

```
OLD (v3.0):
Phase 1 → Phase 2 → Phase 3 → Phase 4 → Phase 5 → Phase 6 → Phase 7

NEW (v3.1):
┌─────────────────────────────────────────────────────────┐
│                                                         │
│  ┌─────────┐    ┌─────────┐    ┌─────────┐             │
│  │ Generate│───▶│ Execute │───▶│  Score  │             │
│  └─────────┘    └─────────┘    └─────────┘             │
│       │                              │                  │
│       ▼                              ▼                  │
│  ┌─────────┐                   ┌─────────┐             │
│  │ Prune   │◀──────────────────│ KeepBest│             │
│  └─────────┘                   └─────────┘             │
│       │                              │                  │
│       ▼                              │                  │
│  ┌─────────┐                        │                  │
│  │Aggregate│────────────────────────┘                  │
│  └─────────┘                                           │
│       │                                                │
│       ▼                                                │
│  Check Confidence                                      │
│  (≥ 0.9?) ──No──▶ Loop                                 │
│      │                                                   │
│      Yes                                                 │
│      ▼                                                   │
│  Synthesize Report                                      │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

### GoT 操作

| 操作 | 工具 | 描述 |
|------|------|------|
| `generate_paths(k)` | `mcp__deep-research__generate_paths` | 生成 k 个探索路径 |
| `score_and_prune` | `mcp__deep-research__score_and_prune` | 评分并保留 top N |
| `aggregate_paths` | `mcp__deep-research__aggregate_paths` | 合并路径 |

### 路径模板

```typescript
// 预定义路径模板 (v3.1)
const PATH_TEMPLATES = {
  academic: {
    focus: 'Academic Research',
    query_pattern: '{topic} academic papers research {year}',
    sources: ['scholar.google.com', 'arxiv.org'],
    weight: 0.3
  },
  industry: {
    focus: 'Industry Practices',
    query_pattern: '{topic} industry report case study',
    sources: ['mckinsey.com', 'gartner.com'],
    weight: 0.25
  },
  policy: {
    focus: 'Policy & Governance',
    query_pattern: '{topic} policy regulation governance',
    sources: ['gov.uk', 'europa.eu'],
    weight: 0.2
  }
  // ... technical, news
};
```

### 多样化策略

| 策略 | 描述 | 使用场景 |
|------|------|----------|
| `orthogonal` | 最小化路径重叠 | 需要全面覆盖 |
| `complementary` | 路径相互增强 | 需要深度交叉验证 |
| `exploratory` | 广泛探索 | 早期研究阶段 |
| `focused` | 深度挖掘 | 特定方面研究 |

---

## MCP 工具架构 (v3.1)

### 新增工具

```typescript
// v3.1 新增: 自动数据处理工具
interface AutoProcessDataInput {
  session_id: string;
  input_dir: string;      // RESEARCH/topic/data/raw/
  output_dir: string;     // RESEARCH/topic/data/processed/
  operations: [
    'fact_extraction',
    'entity_extraction',
    'citation_validation',
    'conflict_detection'
  ];
}
```

### 工具分类

```
MCP Server Tools:
├── GoT Operations (3) - v3.1 激活
│   ├── generate_paths
│   ├── score_and_prune
│   └── aggregate_paths
│
├── Auto-Processing (1) - v3.1 新增
│   └── auto_process_data
│
├── Unified Tools (3)
│   ├── extract (fact + entity)
│   ├── validate (citation + source)
│   └── conflict-detect
│
├── Batch Processing (3)
│   ├── batch-extract
│   ├── batch-validate
│   └── batch-conflict-detect
│
└── State Management (11)
    ├── Session: create, update, get
    ├── Agent: register, update, get_active
    ├── Phase: update, get, checkpoint
    └── Log: log_activity, render_progress
```

### 数据流程变化

**v3.0** (Agent 侧处理):
```
Agent → 读取文件 → 逐个处理 → 提取事实 → 保存
       (占用大量上下文窗口)
```

**v3.1** (服务器侧处理):
```
Agent → 调用 auto_process_data → 服务器批处理 → 返回结果
       (一个命令，无上下文压力)
```

---


## 研究模式 (v3.1)

### Quick Mode (`--fast`)

```bash
/deep-research "简单问题" --fast
```

**特点**:
- 单一路径，无 GoT
- 3-5 个来源
- 5-10 分钟完成
- 适用于明确、简单的问题

**流程**:
```
验证问题 → 生成查询 → 获取来源 → 提取事实 → 输出答案
```

### Deep Mode (默认)

```bash
/deep-research "复杂主题"
```

**特点**:
- GoT 路径优化
- 20+ 个来源
- 30-60 分钟完成
- 适用于复杂、多方面主题

**流程**:
```
初始化 GoT → 生成路径 → 执行 → 评分 → 剪枝 → 聚合 → 循环
             ↓
         终止条件: confidence ≥ 0.9 OR budget exhausted
```

---

## 性能优化 (v3.1)

### Token 使用对比

| 操作 | v3.0 | v3.1 | 节省 |
|------|------|------|------|
| Phase 4 处理 | ~50K | ~5K | 90% |
| 路径优化 | N/A | 智能剪枝 | 30-50% |
| 总体 (Deep) | ~500K | ~300K | 40% |

### 并行执行

```typescript
// v3.1: 并行部署 workers
const paths = await generate_paths({ k: 3 });

// 单次响应，多个 Task 调用
await Promise.all([
  Task({ subagent_type: 'research-worker-v3', prompt: paths[0].query }),
  Task({ subagent_type: 'research-worker-v3', prompt: paths[1].query }),
  Task({ subagent_type: 'research-worker-v3', prompt: paths[2].query })
]);
```

---

## 数据库模式 (v3.1)

### 新增表

```sql
-- GoT 图状态
CREATE TABLE got_nodes (
  id TEXT PRIMARY KEY,
  session_id TEXT,
  path_id TEXT,
  node_type TEXT,  -- 'generate', 'execute', 'score', 'prune', 'aggregate'
  data JSON,
  timestamp TIMESTAMP,
  FOREIGN KEY (session_id) REFERENCES sessions(id)
);

-- GoT 操作历史
CREATE TABLE got_operations (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  session_id TEXT,
  operation TEXT,
  input_data JSON,
  output_data JSON,
  timestamp TIMESTAMP
);
```

---

## 架构决策记录 (ADR)

### ADR-004: 采用 GoT 架构 (v3.1)

**状态**: 已接受

**上下文**: v3.0 的线性流程无法适应复杂研究的动态需求。

**决策**: 采用 Graph of Thoughts 动态路径优化。

**理由**:
- 智能剪枝节省资源
- 聚合提高质量
- 置信度驱动的终止条件

**后果**:
- ✅ 更高的研究质量
- ✅ 更低的 Token 使用
- ⚠️ 需要理解图遍历

### ADR-005: 服务器端数据处理 (v3.1)

**状态**: 已接受

**上下文**: Agent 侧处理占用大量上下文窗口。

**决策**: 将 Phase 4 移至服务器端 (`auto_process_data`)。

**理由**:
- 减少 LLM 调用
- 确定性处理
- 批处理优化

**后果**:
- ✅ 节省 90% Phase 4 Token
- ⚠️ 需要 MCP 工具调用

---

## 迁移指南

### 从 v3.0 升级到 v3.1

1. **删除旧文件**:
   ```bash
   rm commands/deep-research.md
   rm commands/quick-research.md
   rm shared/skills/
   ```

2. **更新配置**:
   ```bash
   # 设置预算限制
   export RESEARCH_BUDGET_LIMIT=500000
   ```

3. **使用新命令**:
   ```bash
   # Quick 模式
   /deep-research "简单问题" --fast

   # Deep 模式
   /deep-research "复杂主题"
   ```

---

## 未来增强

### v3.2 计划

- [ ] 可视化 GoT 图 UI
- [ ] 实时置信度显示
- [ ] 动态预算分配
- [ ] 路径推荐系统
- [ ] 多模态支持 (图片、视频)

### 研究方向

- 更智能的路径生成
- 自适应剪枝阈值
- 跨会话学习
- 协作研究支持

---

**完整实现细节见 [RESEARCH_METHODOLOGY.md](RESEARCH_METHODOLOGY.md)**
